...
def gen_merkle_proof(leaves, pos):
    ...
    for level in range(height):
        ...
        if level_pos % 2 == 0:  # Check if the position is even or odd
            sibling_pos = level_pos + 1
        else:
            sibling_pos = level_pos - 1

        path.append(state[sibling_pos])  # Append the sibling node to the proof path

        # Calculate the hash of internal nodes at this level
        state = list(map(hash_internal_node, state[::2], state[1::2]))

        level_pos = level_pos // 2  # Move to the parent node

    return path  # Return the Merkle proof path
